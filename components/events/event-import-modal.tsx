...
import { EventColumnMappingStep } from './event-column-mapping-step';
import {
  parseImportFile,
...
export function EventImportModal({
  ...
  // Import mutation
  const importMutation = trpc.event.bulkImport.useMutation({
    onSuccess: (result) => {
      ...
      if (errors === 0) {
        toast({ title: `Imported ${created + updated} events successfully`, type: 'success' });
      } else {
        toast({ title: `Imported ${created + updated} events with ${errors} errors`, type: 'info' });
      }
    },
    onError: (error) => {
  ...
  // Check if can proceed to preview (all required fields mapped)
  const mappedFields = mapping.map((m) => m.targetField);
  const canProceedToPreview = REQUIRED_FIELD_VALUES.every((field) =>
              ...
              <div className="flex gap-4">
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    className="accent-primary"
                  />
                  <span className="text-sm">Create or update (by title + date)</span>
                </label>
              </div>
            </div>
            ...
            <div className="border rounded-lg overflow-hidden">
              ...
              <div className="max-h-[300px] overflow-y-auto">
                {validationResults.map((result) => {
                  const title = result.data?.title || parsedRows[result.rowIndex]?.title || '(no title)';
        ...
        {step === 'importing' && (
          <div className="py-8 text-center">
            ...
            <p className="text-muted-foreground">
              Please wait while we import {validCount} events.
            </p>
          </div>
        )}
